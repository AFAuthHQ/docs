...
afauth trust link                                         # 2. link to a human (one-time, ~90 days)
...
- `key.json` — the agent's keypair, the sole credential for pre-claim operations. Back it up with `afauth keys export`.
