...
The trust attestor exposes a small HTTP surface at `trust.afauth.org`. This flow is what every signup to a default (`attested_only`) service requires: agents drive a link request through `/v1/link/*` once, then exchange the resulting binding token for a per-service attestation JWT at `/v1/token` on each request, presenting it as `AFAuth-Attestation`. A service that lacks the header rejects the request with `401 attestation_required`. Services need only fetch the JWKS at `/.well-known/jwks.json` to verify tokens offline.
...
`binding_id` records that the agent has linked. Minting at `/v1/token` is **keyless** — the agent authenticates by signing the mint request with its account key (see below), so the keypair is the only secret it must keep. The `binding_token` is a legacy bearer credential retained for backward compatibility during the migration to signed minting; new agents do not need to store it. Bindings are long-lived (90 days by default) and revocable by the human at any time. `binding_token_expires_at` is a Unix timestamp.
...
<Note>
During the migration window the attestor also accepts the legacy `Authorization: Bearer <binding_token>` form. A §5 signature takes precedence when both are present. New agents should sign; the bearer path will be removed in a later release.
</Note>
...
After revocation, all subsequent `/v1/token` calls for the binding's agent DID — whether signed by its key or presenting its legacy token — return `403 binding_revoked`. Previously-issued JWTs continue to verify until their `exp` (≤ 900 seconds out). To shorten that window further, services may operate a short-lived `iat` lower bound at their discretion.
...
The per-binding daily cap (`TRUST_PER_BINDING_DAILY_TOKEN_LIMIT`, default 10,000) prevents a compromised binding token from acting as an unlimited attestation oracle, while leaving headroom for the §10.7 re-mint cadence of an agent active across many services.
