...
| `principal_already_registered` | `409` | A service enforcing per-principal uniqueness (§10.4.4) received an attested signup whose `(iss, sub_h)` already holds an account ("same human, same bucket") | §10.4.4 | Stop — this human already has an account here; use the existing one |
...
- `principal_already_registered` — this human already has an account here; "one account per human" (§10.4.4). Use the existing one (don't spin up another agent).
