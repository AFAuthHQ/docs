...
**Status:** Required for v0.1 final. The vectors below are the minimum a v0.1 implementation MUST pass to claim conformance. The full §C.1 through §C.6 corpus (plus the optional §C.7 trust-attestation vectors for §10) now ships under [`../vectors/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/), and an executable harness at [`../harness/run.js`](https://github.com/AFAuthHQ/spec/blob/main/harness/run.js) verifies every committed vector against a reference verifier.
