---
title: "Specification"
description: "AFAuth Protocol v0.1 — normative specification."
---

<Info>This page is auto-synced from [`AFAuthHQ/spec/spec/core.md`](https://github.com/AFAuthHQ/spec/blob/main/spec/core.md). Do not edit here.</Info>
**Version:** 0.1
**Date:** 2026-05-18
**Status:** Stable; comments and proposals welcome.
**Editors:** AFAuth contributors
**License:** [CC-BY 4.0](https://github.com/AFAuthHQ/spec/blob/main/LICENSE)
...
The full JSON Schema is provided alongside this specification at [`../schemas/well-known.json`](https://github.com/AFAuthHQ/spec/blob/main/schemas/well-known.json). Implementations MUST treat unknown fields as opaque (forward-compatibility).
...
**Status:** Required for v0.1 conformance. The vectors below are the minimum a v0.1 implementation MUST pass to claim conformance. The full §C.1 through §C.6 corpus (plus the optional §C.7 trust-attestation vectors for §10) now ships under [`../vectors/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/), and an executable harness at [`../harness/run.js`](https://github.com/AFAuthHQ/spec/blob/main/harness/run.js) verifies every committed vector against a reference verifier.

A reference test-only Ed25519 keypair is published alongside this specification at [`../vectors/keypair.json`](https://github.com/AFAuthHQ/spec/blob/main/vectors/keypair.json). The keypair is for protocol testing only and MUST NOT be used in production. The associated `did:key` identifier is:
...
Vectors are published as machine-readable JSON files under [`../vectors/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/) alongside this specification. Each vector file includes a `description`, the input under test, the expected output, and a reference to the section of this specification that it exercises. A minimum-viable conformance harness lives at [`../harness/run.js`](https://github.com/AFAuthHQ/spec/blob/main/harness/run.js); it loads every vector, runs the §C.1 and §C.2 checks against a reference verifier, and exits non-zero on any failure. Independent implementations are encouraged to contribute additional vectors via the proposals process.
