---
title: "Conformance"
description: "Conformance criteria for agent and service roles."
---

<Info>This page is auto-synced from [`AFAuthHQ/spec/spec/conformance.md`](https://github.com/AFAuthHQ/spec/blob/main/spec/conformance.md). Do not edit here.</Info>
**Status:** Working draft alongside `core.md`. The §C.1 through §C.6 test-vector corpus and the executable harness at [`../harness/run.js`](https://github.com/AFAuthHQ/spec/blob/main/harness/run.js) ship in this repository; an implementation that passes every committed vector through its own verifier may claim v0.1 conformance for its declared role.

This document describes what an AFAuth implementation must support to call itself "AFAuth-conformant." It is the authoritative reference for that claim; the protocol specification ([`core.md`](/spec/core)) provides the underlying normative requirements.
...
The Appendix C corpus ships under [`../vectors/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/):

- **§C.1, §C.2** — [`vectors/signatures/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/signatures/): canonical input + reference signatures.
- **§C.3** — [`vectors/discovery/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/discovery/): well-formed, forward-compatible, and malformed discovery documents.
- **§C.4** — [`vectors/recipients/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/recipients/): per-type normalisation rules with canonical forms.
- **§C.5** — [`vectors/errors/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/errors/): one envelope fixture per §11.3 reserved code.
- **§C.6** — [`vectors/replay-window/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/replay-window/): expired, future-dated, replay-invariant, and cross-keyid sequences.
- **§C.7** — [`vectors/trust-attestation/`](https://github.com/AFAuthHQ/spec/tree/main/vectors/trust-attestation/): `afauth-trust` attestation JWTs — well-formed, runtime-only (no `sub_h`), and malformed `sub_h` cases (§10). Applies only to implementations that consume attestations.

The executable harness at [`../harness/run.js`](https://github.com/AFAuthHQ/spec/blob/main/harness/run.js) runs every fixture against a reference verifier and exits non-zero on any failure. It exports its primitives (`buildCanonicalInput`, `verifySignature`, `checkDiscoveryDocument`, `normaliseRecipient`, …) so independent implementations can reuse them. Contributions of additional vectors are welcome via the proposals process (see [`../proposals/`](https://github.com/AFAuthHQ/spec/blob/main/proposals/README.md)).
