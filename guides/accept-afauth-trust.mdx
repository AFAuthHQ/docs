...
<Note>
Listing `afauth-trust` is one of several supported attestor configurations. See [the trust attestor concept](/concepts/trust-attestor) for what a trust attestation actually proves, and [the broader attestation concept](/concepts/attestation) for HMAC + multi-attestor patterns.
</Note>
...
<Step title="Configure JwksAttestor in your Server">

`@afauthhq/server` (`0.2.0+`) ships a `trustAttestor()` factory that bakes in the `afauth-trust` issuer, the published JWKS URL, and EdDSA — one line in your `Server` config:
...
Then drive the link flow (see [Link your agent to a human](/guides/link-to-a-human)) to receive a trust attestation, and retry the signed request. The agent's request will now carry an `AFAuth-Attestation` header and your service will accept it.
