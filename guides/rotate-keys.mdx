...
<Note>
**`did:key` changes the account identifier on rotation** — the identifier *is* the key, so external references to the old DID stop resolving; a caller holding the old DID updates its reference from the rotation response. Agent identities are `did:key` and AFAuth has no stable-identifier agent method, so recovery is owner-driven (revoke + re-key), not a persistent DID. See [Identity and keys](/concepts/identity-and-keys).
</Note>
