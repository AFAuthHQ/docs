...
It's the OAuth refresh pattern applied to attestation: the agent's **live binding at the attestor is the refresh grant** — it re-mints by signing `/v1/token` with its account key — the **attestation is the short-lived access token**, and revoking the binding kills the access within the access token's lifetime.
