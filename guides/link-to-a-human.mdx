...
Some AFAuth services declare `unclaimed_mode = "attested_only"` and require an [attestation](/concepts/attestation) on each signed request — a signal that a verified human stands behind the agent. This guide walks you through producing that signal using the canonical trust attestor at [trust.afauth.org](https://trust.afauth.org).

<Note>
You only need this for services that accept `afauth-trust` in their `billing.accepted_attestors`. Services that allow `unclaimed_mode = "free"` don't require attestation — your signed request alone suffices.
</Note>
...
3. **Agent ↔ Trust:** poll until confirmed, receive a long-lived **binding token**, and mint short-lived per-service JWTs as needed.

You do this **once** per human/agent pair — the binding token persists (90 days by default). Each per-service JWT is fresh, audience-bound, and ≤ 900 s lived.
...
`link_url` is what the human visits. `req_id` is the opaque handle you'll use to poll. The whole request expires in **10 minutes** (`expires_in: 600`) — make sure the human can act within that window.
...
<Step title="Poll for confirmation, get a binding token">
...
// Store these somewhere durable. The token is delivered ONCE.
saveBindingToken(binding.binding_token, binding.binding_token_expires_at);
```

The binding token is the agent's secret credential at the trust attestor — treat it like an API key (encrypted at rest, never logged). Binding tokens are long-lived (90 days by default). The human can revoke any binding from `trust.afauth.org/account`.
...
The reference CLI (`v0.3.0+`) wraps the same flow under `afauth trust`:
...
If the binding token is lost or compromised:

1. The human signs in at `trust.afauth.org/account` and revokes the binding.
2. The agent starts a new link request (step 1 above) — the human confirms it the same way.
