...
- **`account_id`** — A service-local, opaque account identifier, distinct from any key and **stable across key rotation**, so one account can hold several agent credentials. → [How it works](/concepts/how-it-works)
...
- **Account states** — `UNCLAIMED` (created, no owner) → `INVITED` (invitation pending) → `CLAIMED` (owned); plus `EXPIRED` (unclaimed past TTL) and `ARCHIVED` (owner-deleted). → [The ceremony](/concepts/ceremony)
...
<CardGroup cols={2}>
  <Card title="How it works, end to end" icon="map" href="/concepts/how-it-works">
