...
3. Resolves the public key from `keyid` (decodes `did:key`; the v0.1 reference `Verifier` resolves `did:key` only — `did:web` resolution is an opt-in hook the reference Worker does not ship).
...
## Replay protection — scoped to `keyid`, not account

The seen-nonce set is keyed by `(keyid, nonce)`, **not** `(account_did, nonce)`. This matters for `did:web` accounts that rotate their verification keys: one account identifier may present multiple keyids over its lifetime, and each keyid maintains its own nonce space. A nonce reused under a freshly-rotated key isn't a replay against the previous key's space.
