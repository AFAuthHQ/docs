...
AFAuth doesn't replace OAuth, MCP, or A2A — it plugs into them; [Migrate from OAuth or API keys](/guides/migrate-from-oauth) shows the incremental path for an existing service. Attestation is different: AFAuth defines its own §10 attestation hook and ships a default trust attestor (`afauth-trust`), so spam-resistance works out of the box, while still letting third-party platform and commerce attestors (Entra, Stripe, FIDO AP2) plug in alongside it.
...
- [Security model](/concepts/security-model) — what it protects against, what it assumes, what it leaves to services.
