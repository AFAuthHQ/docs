...
<Card title="How it works, end to end" icon="map" href="/concepts/how-it-works" horizontal>
...
- New to the protocol? Read [How it works, end to end](/concepts/how-it-works), then [Why AFAuth](/concepts/why-afauth).
...
**Spec v0.1 — stable.** The reference [TypeScript SDK](https://www.npmjs.com/org/afauthhq) (npm), [Go CLI](https://github.com/AFAuthHQ/cli/releases) (Homebrew + `install.sh` + `go install`), trust attestor, and registry all pass the conformance harness against the v0.1 vectors. See the [changelog](/changelog/overview) for what ships in each release.
