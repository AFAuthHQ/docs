...
**Spec v0.1 — working draft.** Reference implementations pass the conformance harness against the v0.1 vectors. APIs may evolve based on real-world integration feedback. The reference SDK ships on npm as [`@afauthhq/{agent,server,worker}@0.2.0`](https://www.npmjs.com/org/afauthhq) + [`@afauthhq/core@0.1.0`](https://www.npmjs.com/package/@afauthhq/core) — `0.2.0` adds the AFAP-0006 `TrustClient` + `trustAttestor()` surface and account-expiry sweep, and a follow-up adds the `defineService` factory that wires `attested_only` + `trustAttestor()` by default for spam-resistant integrations. The reference CLI ships as [`afauthhq/cli`](https://github.com/AFAuthHQ/cli/releases) with the `afauth trust` subcommand; `afauth signup` now auto-mints attestations when a service requires them (Homebrew + `install.sh` + Go install all live).
