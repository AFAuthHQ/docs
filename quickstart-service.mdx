...
completion, and key rotation — but with three spam-resistance switches
...
3. Per-principal uniqueness (§10.4.4) is enforced.

The net effect: un-attested implicit signups are rejected at the wire,
and a second account for a human who already has one is rejected with
`409 principal_already_registered` — all of a human's agents carry the
same per-service pseudonym `sub_h` (§10.4), so one human is one bucket
regardless of how many keypairs they spin up. (Pass `subHUniqueness: false`
if you run agent fleets and want many agents per human.)
...
- Swap `Memory*` stores for durable backends. `@afauthhq/worker` ships `D1AccountStore`, `D1SubHUniquenessStore` (atomic per-principal uniqueness), `KvNonceStore`, `KvRevocationList`. The default `MemorySubHUniquenessStore` is process-local — supply `D1SubHUniquenessStore` so "one account per human" holds across instances and restarts. See [Deploy to Cloudflare Workers](/guides/deploy-cloudflare-worker).
